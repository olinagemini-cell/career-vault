...
├── 工作流文档/                        ← 工作流规则与配置
│   ├── CLAUDE.md                      ← 你现在读的这份
│   ├── resume_rules.md                ← 简历规则
│   └── cc_to_hermes_delivery.md       ← 工作流协议
│
├── 策略与复盘/                        ← 策略文档、市场分析、诊断报告
│   ├── 跳槽进展诊断与行动方案_*.md
│   ├── linkedin_profile_改写方案_*.md
│   ├── 投递策略复盘与横向赛道扩展.md
│   └── (其他策略文档)
│
├── 投递管理/                          ← 投递跟踪与 outreach 模板
│   ├── 投递跟踪表.md                  ← 投递状态跟踪（唯一版本）
│   └── outreach/                      ← LinkedIn/猎聘 reach out 模板
│
├── 素材库/                            ← 职业履历素材（SSOT）
...
- 投递跟踪**只维护** `投递管理/投递跟踪表.md`，不要创建重复文件。
- 策略文档、诊断报告统一放 `策略与复盘/`，不要散落根目录。
