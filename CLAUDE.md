...
飞书多维表格是CC和Hermes的任务总线。CC启动时必须先查飞书。

- **Base**: `FEAZbWbPoaJGnPs7DZAcsuWvndb`
- **配置**: `feishu_config.md`（App凭据、表ID、字段ID映射）
- **协议**: [飞书文档](https://rcn139sraus1.feishu.cn/docx/TOn1dPNoDocBiXxbqhUci9flnug)（飞书实时版，GitHub `hermes_to_cc_protocol.md` 为备份）
- **Hermes角色**: `hermes_new_role.md`（发送给Hermes的职责说明）
...
1. 查飞书每日日志 → 看Hermes做了什么
2. 查跳槽管线表 → 筛选状态="值得投"的JD
3. 查碎片收集区 → 读"待处理"碎片
4. 汇总呈现给olina → JD整合归类 → 写简历 → 输出桌面 → 更新飞书状态 + 写每日日志
